...
<script>
(function () {
    const NOTIF_API = <?= json_encode(rtrim((defined('STUDENT_URL') ? STUDENT_URL : ''), '/') . '/api/notifications.php') ?>;
    const CSRF = <?= json_encode($__topbar_csrf) ?>;
    const ICONS = {
        welcome: 'fa-hand-sparkles', transfer_in: 'fa-arrow-down', transfer_out: 'fa-paper-plane',
        topup: 'fa-circle-plus', withdraw: 'fa-money-bill-wave', payment: 'fa-cart-shopping',
        fee_waiver: 'fa-hand-holding-dollar', general: 'fa-bell',
    };
    ...
    // On narrow screens, anchoring to the bell (position:absolute; right:0)
    // pins the panel's right edge to the bell instead of the screen, so it
    // reads off-center. Below the breakpoint, switch to fixed positioning
    // with equal left/right margins from the viewport itself.
    ...
    // back from another tab where a transfer/payment just landed).
    document.addEventListener('visibilitychange', function () {
        if (document.visibilityState === 'visible') poll();
    });

    // Lets other pages (transfer.php, cart.php, ...) trigger an instant
    // refresh right after their own action, instead of waiting for the poll.
